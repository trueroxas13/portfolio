# REQUIREMENTS
This is the main file where we record the requirements, which are split between functional and non-functional. Use cases will be driven from these, add or remove the requirements as necessary after reading through the project prompt.

Note: qPay is used to handle transactions in this automated system. It is treated as a separate entity, thus will function as an actor. Qatar Trading Service (QTS) is also an actor since it is outisde the sytem.
...
- Registration management system (to handle registration/renmewal/transfers of ownerships)
...
- The iQVR system must issue a fitness certificate from authorized workshops in Qatar which must be renewed every 2 years of registration
- System must generate a receipt when issuing this certificate, and store the relevant information and workshop data
- The ability to generate a registration sticker
- A system to generate invoices for transfers (paid later by the new owner) and renewals 
...
- Payments are first handled by the system, then forwarded to qPay for further processing/approval
- qPay sends a receipt after the payment is approved, along with approval advice. also sets invoices as paid
...
- Accident case numebrs must be unique
...
- A confiscation order can only be attacahed to one vehicle, however a vehicle can have more than one of such orders
